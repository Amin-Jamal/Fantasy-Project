...
import com.example.fantasyproject.v2ray.dto.AngConfig
import com.example.fantasyproject.v2ray.dto.EConfigType
import com.example.fantasyproject.v2ray.dto.ServerConfig
import com.example.fantasyproject.v2ray.dto.SubscriptionItem
import com.example.fantasyproject.v2ray.dto.V2rayConfig
import com.google.gson.Gson
//import com.jimbovpn.jimbo2023.R
import com.example.fantasyproject.v2ray.V2rayConstants
import com.example.fantasyproject.v2ray.V2rayConstants.ANG_CONFIG
import com.example.fantasyproject.v2ray.V2rayConstants.HTTPS_PROTOCOL
import com.example.fantasyproject.v2ray.V2rayConstants.HTTP_PROTOCOL
import com.example.fantasyproject.v2ray.dto.V2rayConfig.Companion.DEFAULT_SECURITY
import com.example.fantasyproject.v2ray.dto.V2rayConfig.Companion.TLS
import com.example.fantasyproject.v2ray.dto.VmessQRCode
import com.example.fantasyproject.v2ray.extension.idnHost
import com.example.fantasyproject.v2ray.utils.MmkvManager.KEY_SELECTED_SERVER
...
object AngConfigManager {
    ...
    fun migrateLegacyConfig(c: Context): Boolean? {
        try {
            ...
            Log.d("ANGTAG", defaultSharedPreferences.toString())
            val context = defaultSharedPreferences.getString(ANG_CONFIG, "")
            Log.d("ANGTAG", context.toString())
    ...
    private fun copyLegacySettings(sharedPreferences: SharedPreferences) {
        listOf(
            V2rayConstants.PREF_MODE,
            V2rayConstants.PREF_REMOTE_DNS,
            V2rayConstants.PREF_DOMESTIC_DNS,
            V2rayConstants.PREF_LOCAL_DNS_PORT,
            V2rayConstants.PREF_SOCKS_PORT,
            V2rayConstants.PREF_HTTP_PORT,
            V2rayConstants.PREF_LOGLEVEL,
            V2rayConstants.PREF_ROUTING_DOMAIN_STRATEGY,
            V2rayConstants.PREF_ROUTING_MODE,
            V2rayConstants.PREF_V2RAY_ROUTING_AGENT,
            V2rayConstants.PREF_V2RAY_ROUTING_BLOCKED,
            V2rayConstants.PREF_V2RAY_ROUTING_DIRECT,
        ).forEach { key ->
            settingsStorage?.encode(key, sharedPreferences.getString(key, null))
        }
        listOf(
            V2rayConstants.PREF_SPEED_ENABLED,
            V2rayConstants.PREF_PROXY_SHARING,
            V2rayConstants.PREF_LOCAL_DNS_ENABLED,
            V2rayConstants.PREF_ALLOW_INSECURE,
            V2rayConstants.PREF_PREFER_IPV6,
            V2rayConstants.PREF_PER_APP_PROXY,
            V2rayConstants.PREF_BYPASS_APPS,
        ).forEach { key ->
            settingsStorage?.encode(key, sharedPreferences.getBoolean(key, false))
        }
        settingsStorage?.encode(
            V2rayConstants.PREF_SNIFFING_ENABLED,
            sharedPreferences.getBoolean(V2rayConstants.PREF_SNIFFING_ENABLED, true)
        )
        settingsStorage?.encode(
            V2rayConstants.PREF_PER_APP_PROXY_SET,
            sharedPreferences.getStringSet(V2rayConstants.PREF_PER_APP_PROXY_SET, setOf())
        )
    }
            ...
            if (type == EConfigType.CUSTOM) {
                ...
                config.outboundBean?.streamSettings?.let { streamSetting ->
                    ...
                    val allowInsecure = if (vmessBean.allowInsecure.isBlank()) {
                        settingsStorage?.decodeBool(V2rayConstants.PREF_ALLOW_INSECURE) ?: false
                    } else {
    ...
    private fun importConfig(
        ...
        try {
            ...
            val allowInsecure = settingsStorage?.decodeBool(V2rayConstants.PREF_ALLOW_INSECURE) ?: false
    ...
    private fun tryResolveResolveSip002(str: String, config: ServerConfig): Boolean {
        try {
            ...
            Log.d(V2rayConstants.ANG_PACKAGE, e.toString())
            return false
        }
    }
        ...
        try {
            ...
            val removedSelectedServer =
                if (!TextUtils.isEmpty(subid) && !append) {
                    Log.d("ANCOMA", "importBatchConfig FUNCTION -> inside 2nd if statement")
                    MmkvManager.decodeServerConfig(
                        mainStorage?.decodeString(KEY_SELECTED_SERVER) ?: ""
                    )?.let {
                        if (it.subscriptionId == subid) {
                            Log.d("ANCOMA", "importBatchConfig FUNCTION -> inside 3rd if statement")
                            return@let it
                        }
                        return@let null
                    }
                } else {
                    Log.d("ANCOMA", "importBatchConfig FUNCTION -> inside 1st else block")

                    null
                }
            Log.d("ANCOMA", "importBatchConfig FUNCTION -> servers = $servers")
            if(!append) {
