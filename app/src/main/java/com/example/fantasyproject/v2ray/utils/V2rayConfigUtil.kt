...
import com.example.fantasyproject.v2ray.V2rayConstants.ANG_PACKAGE
...
import com.example.fantasyproject.v2ray.dto.V2rayConfig.Companion.DEFAULT_NETWORK
import com.example.fantasyproject.v2ray.dto.V2rayConfig.Companion.HTTP
...
object V2rayConfigUtil {
    ...
    private fun getV2rayNonCustomConfig(context: Context, outbound: V2rayConfig.OutboundBean): Result {
        ...
        v2rayConfig.log.loglevel = settingsStorage?.decodeString(V2rayConstants.PREF_LOGLEVEL)
            ?: "warning"
        ...
        if (settingsStorage?.decodeBool(V2rayConstants.PREF_SPEED_ENABLED) != true) {
    ...
    private fun inbounds(v2rayConfig: V2rayConfig): Boolean {
        try {
            val socksPort = Utils.parseInt(settingsStorage?.decodeString(V2rayConstants.PREF_SOCKS_PORT), V2rayConstants.PORT_SOCKS.toInt())
            val httpPort = Utils.parseInt(settingsStorage?.decodeString(V2rayConstants.PREF_HTTP_PORT), V2rayConstants.PORT_HTTP.toInt())

            v2rayConfig.inbounds.forEach { curInbound ->
                if (settingsStorage?.decodeBool(V2rayConstants.PREF_PROXY_SHARING) != true) {
                    //bind all inbounds to localhost if the user requests
                    curInbound.listen = "127.0.0.1"
                }
            }
            v2rayConfig.inbounds[0].port = socksPort
            val fakedns = settingsStorage?.decodeBool(V2rayConstants.PREF_FAKE_DNS_ENABLED)
                ?: false
            val sniffAllTlsAndHttp = settingsStorage?.decodeBool(V2rayConstants.PREF_SNIFFING_ENABLED, true)
                ?: true
...
//            }
    ...
    private fun fakedns(v2rayConfig: V2rayConfig) {
        if (settingsStorage?.decodeBool(V2rayConstants.PREF_FAKE_DNS_ENABLED) == true) {
    ...
    private fun routing(v2rayConfig: V2rayConfig): Boolean {
        try {
            routingUserRule(
                settingsStorage?.decodeString(V2rayConstants.PREF_V2RAY_ROUTING_AGENT)
                ?: "", V2rayConstants.TAG_AGENT, v2rayConfig)
            routingUserRule(
                settingsStorage?.decodeString(V2rayConstants.PREF_V2RAY_ROUTING_DIRECT)
                ?: "", V2rayConstants.TAG_DIRECT, v2rayConfig)
            routingUserRule(
                settingsStorage?.decodeString(V2rayConstants.PREF_V2RAY_ROUTING_BLOCKED)
                ?: "", V2rayConstants.TAG_BLOCKED, v2rayConfig)

            v2rayConfig.routing.domainStrategy = settingsStorage?.decodeString(V2rayConstants.PREF_ROUTING_DOMAIN_STRATEGY)
                ?: "IPIfNonMatch"
//            v2rayConfig.routing.domainMatcher = "mph"
            val routingMode = settingsStorage?.decodeString(V2rayConstants.PREF_ROUTING_MODE) ?: ERoutingMode.GLOBAL_PROXY.value
    ...
    private fun customLocalDns(v2rayConfig: V2rayConfig): Boolean {
        try {
            ...
            if (v2rayConfig.inbounds.none { e -> e.protocol == "dokodemo-door" && e.tag == "dns-in" }) {
                ...
                val localDnsPort = Utils.parseInt(settingsStorage?.decodeString(V2rayConstants.PREF_LOCAL_DNS_PORT), V2rayConstants.PORT_LOCAL_DNS.toInt())
