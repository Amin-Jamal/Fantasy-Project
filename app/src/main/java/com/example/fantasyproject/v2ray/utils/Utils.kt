...
object Utils {
    ...
    fun tryDecodeBase64(text: String): String? {
        try {
            ...
            Log.i(BuildConfig.APPLICATION_ID, "Parse base64 standard failed $e")
        }
        try {
            return Base64.decode(text, Base64.NO_WRAP.or(Base64.URL_SAFE)).toString(charset("UTF-8"))
        } catch (e: Exception) {
            Log.i(BuildConfig.APPLICATION_ID, "Parse base64 url safe failed $e")
        }
        return null
    }
    ...
    fun getRemoteDnsServers(): List<String> {
        val remoteDns = settingsStorage?.decodeString(V2rayConstants.PREF_REMOTE_DNS) ?: V2rayConstants.DNS_AGENT
        val ret = remoteDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(V2rayConstants.DNS_AGENT)
        }
        return ret
    }

    fun getVpnDnsServers(): List<String> {
        val vpnDns = settingsStorage?.decodeString(V2rayConstants.PREF_VPN_DNS)
            ?: settingsStorage?.decodeString(V2rayConstants.PREF_REMOTE_DNS)
            ?: V2rayConstants.DNS_AGENT
    ...
    fun getDomesticDnsServers(): List<String> {
        val domesticDns = settingsStorage?.decodeString(V2rayConstants.PREF_DOMESTIC_DNS) ?: V2rayConstants.DNS_DIRECT
        val ret = domesticDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(V2rayConstants.DNS_DIRECT)
        }
        return ret
    }
...
//        context.toast(R.string.toast_services_stop)
    ...
    fun userAssetPath(context: Context?): String {
        if (context == null)
            return ""
        val extDir = context.getExternalFilesDir(AppConstant.DIR_ASSETS)
                ?: return context.getDir(AppConstant.DIR_ASSETS, 0).absolutePath
    ...
    fun getLocale(context: Context): Locale =
        when (settingsStorage?.decodeString(V2rayConstants.PREF_LANGUAGE) ?: "auto") {
            "auto" ->  getSysLocale()
            "en" -> Locale("en")
            "zh-rCN" -> Locale("zh", "CN")
            "zh-rTW" -> Locale("zh", "TW")
