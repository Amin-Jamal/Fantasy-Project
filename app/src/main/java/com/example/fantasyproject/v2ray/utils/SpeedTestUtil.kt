...
import com.example.fantasyproject.v2ray.extension.responseLength
...
object SpeedTestUtil {
    ...
    fun realPing(config: String): Long {
        return try {
            ...
            Log.d(BuildConfig.APPLICATION_ID, "realPing: $e")
            -1L
        }
    }
    ...
    fun socketConnectTime(url: String, port: Int): Long {
        try {
            ...
            Log.d(BuildConfig.APPLICATION_ID, "socketConnectTime IOException: $e")
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return -1
    ...
    fun testConnection(context: Context, port: Int): String {
        ...
        try {
            ...
            // network exception
            Log.d(BuildConfig.APPLICATION_ID, "testConnection IOException: " + Log.getStackTraceString(e))
            result = context.getString(R.string.connection_test_error, e.message)
        } catch (e: Exception) {
            // library exception, eg sumsung
            Log.d(BuildConfig.APPLICATION_ID, "testConnection Exception: " + Log.getStackTraceString(e))
            result = context.getString(R.string.connection_test_error, e.message)
        } finally {
            conn?.disconnect()
        }
