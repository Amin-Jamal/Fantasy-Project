...
import com.example.fantasyproject.databinding.ActivityBypassListBinding
...
import com.example.fantasyproject.v2ray.extension.GemApplication
import com.example.fantasyproject.v2ray.utils.AppManagerUtil
import com.example.fantasyproject.v2ray.utils.Utils
//import com.jimbovpn.jimbo2023.databinding.ActivityBypassListBinding
...
class PerAppProxyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val blacklist = defaultSharedPreferences.getStringSet(V2rayConstants.PREF_PER_APP_PROXY_SET, null)
...
//                }
        ...
        binding.switchPerAppProxy.setOnCheckedChangeListener { _, isChecked ->
            defaultSharedPreferences.edit().putBoolean(V2rayConstants.PREF_PER_APP_PROXY, isChecked).apply()
        }
        binding.switchPerAppProxy.isChecked = defaultSharedPreferences.getBoolean(V2rayConstants.PREF_PER_APP_PROXY, false)

        binding.switchBypassApps.setOnCheckedChangeListener { _, isChecked ->
            defaultSharedPreferences.edit().putBoolean(V2rayConstants.PREF_BYPASS_APPS, isChecked).apply()
        }
        binding.switchBypassApps.isChecked = defaultSharedPreferences.getBoolean(V2rayConstants.PREF_BYPASS_APPS, false)
...
//        binding.actionBar.btMenu.visibility = View.GONE
//        binding.actionBar.btBack.visibility = View.VISIBLE
//        binding.actionBar.btBack.setOnClickListener {
//            finish()
//        }
    ...
    override fun onPause() {
        super.onPause()
        adapter?.let {
            defaultSharedPreferences.edit().putStringSet(V2rayConstants.PREF_PER_APP_PROXY_SET, it.blacklist).apply()
        }
    }
...
//        toast(R.string.msg_downloading_content)
        val url = V2rayConstants.androidpackagenamelistUrl
        lifecycleScope.launch(Dispatchers.IO) {
            val content = Utils.getUrlContext(url, 5000)
            launch(Dispatchers.Main) {
//                Log.d(GEM_PACKAGE, content)
    ...
    private fun selectProxyApp(content: String, force: Boolean): Boolean {
        try {
            val proxyApps = if (TextUtils.isEmpty(content)) {
                Utils.readTextFromAssets(GemApplication, "proxy_packagename.txt")
            } else {
                content
            }
...
//        GemLog.print("attachBaseContext", "attachBaseContext")
        ...
        config?.let {
            ...
            // myContext?.resources?.updateConfiguration(it,resources.displayMetrics)
        }
