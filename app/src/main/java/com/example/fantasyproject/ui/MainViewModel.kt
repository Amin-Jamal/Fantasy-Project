...
import com.example.fantasyproject.App
import com.example.fantasyproject.AppConstant
import com.example.fantasyproject.AppConstant.ANG_PACKAGE
import com.example.fantasyproject.R
import com.example.fantasyproject.databinding.DialogConfigFilterBinding
import com.example.fantasyproject.utils.SpeedtestUtil
...
import com.example.fantasyproject.v2ray.utils.MmkvManager.KEY_ANG_CONFIGS
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun startListenBroadcast() {
        ...
        getApplication<App>().registerReceiver(mMsgReceiver, IntentFilter(AppConstant.BROADCAST_ACTION_ACTIVITY))
        MessageUtil.sendMsg2Service(getApplication(), AppConstant.MSG_REGISTER_CLIENT, "")
    }
    ...
    fun testAllTcping() {
        ...
        getApplication<App>().toast("Testing...")
    ...
    fun testAllRealPing() {
        MessageUtil.sendMsg2TestService(getApplication(), AppConstant.MSG_MEASURE_CONFIG_CANCEL, "")
        MmkvManager.clearAllTestDelayResults()
        updateListAction.value = -1 // update all
        ...
        viewModelScope.launch(Dispatchers.Default) { // without Dispatchers.Default viewModelScope will launch in main thread
            for (item in serversCache) {
                ...
                if (config.status) {
                    MessageUtil.sendMsg2TestService(getApplication(), AppConstant.MSG_MEASURE_CONFIG, Pair(item.guid, config.content))
                }
            }
        }
    }
    ...
    fun testCurrentServerRealPing() {
        MessageUtil.sendMsg2Service(getApplication(), AppConstant.MSG_MEASURE_DELAY, "")
    }
...
//            }.show()
    ...
    fun removeDuplicateServer() {
        ...
        reloadServerList()
        getApplication<App>().toast(getApplication<App>().getString(R.string.title_del_duplicate_config_count, deleteServer.count()))
    }

    private val mMsgReceiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                AppConstant.MSG_STATE_RUNNING -> {
                    isRunning.value = true
                }
                AppConstant.MSG_STATE_NOT_RUNNING -> {
                    isRunning.value = false
                }
                AppConstant.MSG_STATE_START_SUCCESS -> {
                    getApplication<App>().toast(R.string.toast_services_success)
                    isRunning.value = true
                }
                AppConstant.MSG_STATE_START_FAILURE -> {
                    getApplication<App>().toast(R.string.toast_services_failure)
                    isRunning.value = false
                }
                AppConstant.MSG_STATE_STOP_SUCCESS -> {
                    isRunning.value = false
                }
                AppConstant.MSG_MEASURE_DELAY_SUCCESS -> {
                    updateTestResultAction.value = intent.getStringExtra("content")
                }
                AppConstant.MSG_MEASURE_CONFIG_SUCCESS -> {
